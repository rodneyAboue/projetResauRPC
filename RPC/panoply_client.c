...
panoplyprog_1(char *host)
{
	...
	if (clnt == NULL) {
		...
		char collection[5][20]={"Vestes","Robes","Manteaux","Sacs","Combis & Smokings"};

		for(int i = 0;i<5;i++)
		{	
			strcpy(creercollection_1_arg.nomCollection,collection[i]);
		...
		{
			/*result12= affichercollection_1(&i, clnt);
			if (result12 == (collection *)NULL) {
				clnt_perror (clnt, "call failed");
			}*/
			printf("*******************************\n");
			printf("COLLECTION %d\n",i);
			//printf("NOM :%s\n",result12->nomCollection);
			printf("*******************************\n");
		}
